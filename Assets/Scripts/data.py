...
for i, file_path in enumerate(file_paths):
    ...
    fig, (ax2, ax1) = plt.subplots(2, 1, figsize=(10, 12), sharex=True)  # 交换 ax1 和 ax2 的位置

    # Plotting the time comparison line chart in the first subplot (now on top)
    ax2.plot(time, time_comparison, marker='', linestyle='-', color='r', label='Time Comparison (0-180s)')
    ax2.fill_between(time, time_comparison, alpha=0.3, color='r')
    ax2.set_ylabel('Time Comparison (0 or 1)')
    ax2.set_ylim(0, 1.1)
    ax2.set_yticks([0, 1])
    ax2.legend()
    ax2.grid()
    ...
    ax1.plot(time, vection_response, marker='', linestyle='-', color='b', label='Vection Response')
    ax1.fill_between(time, vection_response, alpha=0.3, color='b')
    ax1.set_xlabel('Time (s)')
    ax1.set_ylabel('Vection Response (0 or 1)')
    ax1.set_ylim(0, 1.1)
    ax1.set_yticks([0, 1])
    ax1.set_title(f'Vection Response vs Time (Experiment {i+1})')
    ax1.legend()
    ax1.grid()

    # Set x-axis limit to include the full range of time values
    ax1.set_xlim(left=min(time) - 5, right=max(time) + 5)
    ...
    # Draw dashed lines connecting the two subplots at 0 and 180 seconds
    ax2.plot([0, 0], [0, 1], color='purple', linestyle='--', transform=fig.transFigure, clip_on=False)
    ax2.plot([180, 180], [0, 1], color='purple', linestyle='--', transform=fig.transFigure, clip_on=False)

    # Annotate the first and last values in the Time column on the x-axis
    first_time_value = min(time)
    last_time_value = max(time)
    ax1.text(first_time_value, -0.1, f'{first_time_value:.2f} s', color='black', fontsize=9, horizontalalignment='center')
    ax1.text(last_time_value, -0.1, f'{last_time_value:.2f} s', color='black', fontsize=9, horizontalalignment='center')
    ...
    if any(vection_response == 1):
        ...
        ax1.axvline(x=first_occurrence_time, color='r', linestyle='--', label=f'First Response=1 at {first_occurrence_time:.2f} s')
        ax1.text(first_occurrence_time, 1.05, f'{first_occurrence_time:.2f} s', color='r', fontsize=9, horizontalalignment='center')
    ...
    ax1.text(0.95, 0.8, f'Total Time (Response=1): {total_duration_1:.2f} s',
